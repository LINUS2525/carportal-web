...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`https://carportal-web.onrender.com/myJobs/${user?.email}`)
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        setIsLoading(false);
  ...
  const handleSearch = () => {
    const filter = jobs.filter(
      (job) =>
        job.carName.toLowerCase().indexOf(searchText.toLowerCase()) !== -1
    );
    //console.log(filter)
    setJobs(filter);
    setIsLoading(false);
  ...
  const handleDelete = (id) => {
   // console.log(id)
    fetch(`https://carportal-web.onrender.com/job/${id}`, {
        method: "DELETE"
    })
    .then((res) => res.json)
    .then((data) => {
  ...
  return (
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
      <div className="my-jobs-container">
        <h1 className="text-center p-4">All My Cars</h1>
